/*
**	该部分代码实现了一元多项式的减法法, 包括顺序表和链表两部分，顺序表部分弃之不用，可以忽略
**	使用链表结构 定义了数据类型 Node 具体见文件 calc.h
**	减法法结果可以保存在新建的表中，也可以保存在输入的表中。
...
void SubList(List list1, List list2, List &list3){
	...
	while(a != NULL && b != NULL){//合并两表的操作
		...
		if(a->exp - b->exp > 1e-8){			//	指数不同，在新表上插入指数较大的。指针后移
			...
			cur->next = node;
			cur = cur->next;
		}else if(abs(a->exp - b->exp) < 1e-8){//	指数相同 看系数和，和为零删除节点，否则插入节点。
		...
		}else if(a->exp - b->exp < -1*1e-8){		// 减数系数取相反数
